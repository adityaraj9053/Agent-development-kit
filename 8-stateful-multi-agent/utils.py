...
def update_interaction_history(session_service, app_name, user_id, session_id, entry):
    ...
    try:
        ...
        updated_state = session.state.copy() # session.state have user_name, purchased_courses, interaction_history. shallow Copy it with values.
        updated_state["interaction_history"] = interaction_history #now, in updated_state choose field with "interaction_history" and store all history.
            ...
            for idx, interaction in enumerate(interaction_history, 1):
                ...
                if isinstance(interaction, dict):
                    ...
                    if action == "user_query":
                        ...
                        """interaction = {
                            `"action": "feedback",
                            "timestamp": "2025-05-30 14:22",
                            "rating": "positive",
                            "comment": "The explanation was very clear!"
                        }
                        idx = 3  # Example index`
                     What the code does:
                    action is "feedback", which is not "user_query" or "agent_response", so it goes to the else block.It 
                    extracts all key-value pairs except "action" and "timestamp", resulting in:
                    
                    details = "rating: positive, comment: The explanation was very clear!"
                    
                    output:
                    
                      3. feedback at 2025-05-30 14:22 (rating: positive, comment: The explanation was very clear!)"""
                else:
