...
def refund_course(tool_context: ToolContext) -> dict: #  It ia class, acts as a container that provides access to shared resources or state during tool (or function) execution
    """
    ...
    course_ids = [
        course["id"] for course in current_purchased_courses if isinstance(course, dict)  # current_purchased_courses list ke har element (course) ko check karna — agar wo dictionary hai (isinstance(course, dict)), toh usmein se "id" value nikal kar ek naye list course_ids mein daal dena.
    ]
